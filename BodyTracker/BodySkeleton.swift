...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            /// NOTE: Green joints are actively tracked by ARKit. Yellow joints are not tracked. They just follow the motion
            /// of the closet green parent
            ...
            case "jaw_joint", "chin_joint", "left_eye_joint", "left_eyeLowerLid_joint", "left_eyeUpperLid_joint",
                "left_eyeball_joint", "nose_joint", "right_eye _joint", "right_eyeLowerLid_joint", "right_eyeUpperLid_joint", "right_eyeball_joint":
                jointRadius *= 0.2
                jointColor = .yellow
            case _ where jointName.hasPrefix("spine._"):
                jointRadius *= 0.75
    ...
    /// Not an entity that can be visualise
    /// Only provides the parameters that are needed to visualise the bone
    private func createSkeletonBone(bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        ...
        guard let fromJointEntityTransform = bodyAnchor.skeleton.modelTransform(for: ARSkeleton.JointName(rawValue: bone.jointFromName)),
              let toJointEntityTransform = bodyAnchor.skeleton.modelTransform(for: ARSkeleton.JointName(rawValue: bone.jointToName)) else {
            return nil
        }
        
        let rootPosition = simd_make_float3(bodyAnchor.transform.columns.3)
        
        // Realtive to root position i.e. HipJoint
    ...
    private func createBoneEntity(for skeletonBone: SkeletonBone, diamtere: Float = 0.04, color: UIColor = .white) -> Entity {
        let mesh = MeshResource.generateBox(size: [diamtere,diamtere,skeletonBone.length], cornerRadius: diamtere/2)
